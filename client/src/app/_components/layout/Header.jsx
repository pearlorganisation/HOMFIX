...
const [pages,setPages] = useState(false)
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (offset > 50) { // Adjust the threshold as needed
        setIsScrolled(true);
      } else {
        setIsScrolled(false);
      }
  ...
  return (
    <>
    <div className={`h-10 bg-[#4C5C6B] text-white w-full  flex justify-center items-center `}>
    ...
    <div className={`z-50 w-full  bg-[#0E2337] text-white h-20  flex justify-center  ${
          isScrolled ? 'fixed top-0' : ''
        }`}>
      {/* //Header for Desktop*/}

      <div class='hidden w-full  max-w-screen-xl px-5 lg:flex flex-wrap  items-center justify-between gap-2 '>
        <Link href="javascript:void(0)" className='text-2xl font-bold font-sans'>
        ...
        <div id="collapseMenu"
          ...
          <ul
            ...
            <li class='max-lg:border-b border-gray-300 max-lg:py-3 px-3'><Link href='javascript:void(0)'
              class='hover:text-[#007bff]  block font-medium  text-[16px] '>Pricing</Link>
            </li>
        ...
        <div class='flex  lg:max-lg:ml-auto  lg:space-x-6 '>
          ...
          <button
            class='px-4 py-2 flex items-center gap-2 rounded-md font-bold text-white border-2 border-slate-700 bg-slate-700 transition-all ease-in-out duration-300 hover:bg-transparent hover:text-[#007bff]'>
            <RiChat1Line size={24}/> Let's Talk</button>
              ...
              <ul className='space-y-3'>
                ...
                <li className='border-b border-gray-300 py-3 px-3'>
                  <Link href='javascript:void(0)' onClick={()=>{
                    ...
                    Solutions <span> {whoWeServeContent ? <FaAngleUp /> : <FaAngleDown/>}</span>
                  </Link>
                </li>
                {whoWeServeContent &&
                <div class=""> 
...
const FlyoutLink = ({ children, href, FlyoutContent }) => {
  ...
  return (
    ...
    >
      <Link href={"/"} className='relative flex items-center gap-1 font-medium hover:text-[#007bff] text-[16px]'>
        {children} <span>{open ? <FaAngleUp size={15} /> : <FaAngleDown size={15} />}</span>
        <span
...
const SolutionsContent = () => {
  return <div className='min-w-[200px]  rounded-md text-[#0E2337]  text-[15px] font-medium bg-white p-6 shadow-xl flex gap-20 text-start'>
    <div className='flex flex-col gap-2'>
      <div className='text-slate-400 font-normal text-sm'>LEAD MANAGEMENT</div>
      <Link href={"/"} className="hover:underline hover:underline-offset-4  whitespace-nowrap">
    PMO Services
    <div className='font-thin text-sm'>Tech Enabled Project Management</div></Link>
    <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
    Project Financing
    <div className='font-thin text-sm'>Get Financing for your Projects</div></Link>
    </div>
    <div className='flex flex-col gap-2 min-w-[200px] '>
      <div className='text-slate-400 font-normal text-sm'>PROJECT MANAGEMENT</div>
      <Link href={"/"} className="hover:underline hover:underline-offset-4  ">
      Design Management
    <div className='font-thin text-sm'>Manage designs, versions & mark changes </div></Link>
    <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
    Project Financing
    <div className='font-thin text-sm'>Get Financing for your Projects</div></Link>
    </div>
    <div className='flex flex-col gap-2'>
      <div className='text-slate-400 font-normal text-sm min-w-[200px]'>VENDOR MANAGEMENT</div>
      <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
    PMO Services
    <div className='font-thin text-sm'>Tech Enabled Project Management</div></Link>
    <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
    ...
    <div className='flex flex-col gap-2'>
      <div className='text-slate-400 font-normal text-sm'>TASK MANAGEMENT</div>
      <Link href={"/"} className="hover:underline hover:underline-offset-4  whitespace-nowrap">
    PMO Services
    <div className='font-thin text-sm'>Tech Enabled Project Management</div></Link>
    <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
...
const WhoWeServeContent = () => {
  return (
    <div className='text-justify rounded-md text-[#0E2337] text-[15px] font-medium bg-white p-6 shadow-xl flex flex-col gap-2 min-w-[200px]'>
      <Link href={"/"} className="h-10 flex items-center px-2 gap-4 rounded-md hover:bg-gray-50 whitespace-nowrap">
...
const ServicesContent = () => {
  return <div className='min-w-[200px] rounded-md text-[#0E2337]  text-[15px] font-medium  bg-white p-6 shadow-xl flex flex-col  gap-2 text-start'>
    <Link href={"/"} className="hover:underline hover:underline-offset-4  whitespace-nowrap">
    PMO Services
    <div className='font-thin text-sm'>Tech Enabled Project Management</div></Link>
    <Link href={"/"} className="hover:underline hover:underline-offset-4 ">
